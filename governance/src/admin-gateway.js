...
const ACCEPTANCE_SCOPE="cloudflare-dry-run-build-control-plane-v1";
...
export function adminOpenApiPaths(){
  return {
    ...
    "/v1/admin/candidates":{post:{operationId:"createCandidateVersion",summary:"Trigger pinned Cloudflare candidate dry runs",description:"Trigger four non-production Workers Builds pinned to exact Git commits. Only build-and-package dry runs are allowed; no Worker version is uploaded and production traffic is never changed.",security:[{BearerAuth:[]}],requestBody:{required:true,content:{"application/json":{schema:{type:"object",additionalProperties:false,required:["branch","commits"],properties:{branch:{type:"string",minLength:1,maxLength:200},commits:{type:"object",additionalProperties:false,required:["governance","intelligence","compute","expert"],properties:{governance:{type:"string",pattern:"^[A-Fa-f0-9]{40}$"},intelligence:{type:"string",pattern:"^[A-Fa-f0-9]{40}$"},compute:{type:"string",pattern:"^[A-Fa-f0-9]{40}$"},expert:{type:"string",pattern:"^[A-Fa-f0-9]{40}$"}}},label:{type:"string",maxLength:120},reason:{type:"string",maxLength:500}}}}}},responses:{"202":{description:"All four candidate dry-run builds accepted."},"400":{description:"Invalid branch, commit SHA, or request body."},"401":{description:"Unauthorized."},"409":{description:"A center is active; candidate not triggered."},"503":{description:"Runtime context, admin state, Builds API credentials, or safe dry-run trigger unavailable."}}}},
    "/v1/admin/candidates/validate":{post:{operationId:"validateCandidate",summary:"Validate Cloudflare candidate dry runs",description:"Check four dry-run build outcomes, exact branch and commit identity, the non-mutating deploy command, and that production versions/source digests stayed unchanged. No runtime E2E is claimed, and no Worker version is uploaded.",security:[{BearerAuth:[]}],requestBody:{required:true,content:{"application/json":{schema:{type:"object",additionalProperties:false,required:["candidate_id"],properties:{candidate_id:{type:"string",minLength:1,maxLength:160}}}}}},responses:{"200":{description:"Dry-run build control-plane validation PASS receipt."},"202":{description:"One or more candidate builds are still running."},"400":{description:"Invalid candidate_id."},"401":{description:"Unauthorized."},"404":{description:"Candidate not found."},"422":{description:"Candidate validation failed; FAIL receipt stored."},"503":{description:"Admin state, runtime context, or Builds API unavailable."}}}},
...
export async function createCandidateVersion(request,env,ctx,app){
  ...
  try{
    ...
    const manifest={
      candidate_id,candidate_kind:"cloudflare-dry-run-build-set",created_at,label,reason,
...
export async function validateCandidate(request,env,ctx,app){
  ...
  try{
    ...
    if(candidate?.manifest?.candidate_kind!=="cloudflare-dry-run-build-set")return json({ok:false,error:"LEGACY_CANDIDATE_NOT_DRY_RUN_BUILD",http_status:409},409);
