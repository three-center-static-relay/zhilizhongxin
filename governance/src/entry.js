...
async function augmentBaseResponse(request, env, ctx) {
  ...
  if (url.pathname === "/health") return json({ ...body, ai_assist: true, routing_mode: routing.mode, policy_kernel: POLICY_KERNEL_VERSION, fallback_selftests: true });
  ...
  if (url.pathname === "/v1/capabilities" || url.pathname === "/capabilities") {
    return json({ ...body, capabilities: { ...(body.capabilities || {}), governance_ai_assist: true, workers_ai_random_failover: true, openrouter_paid_ranked_failover: true, web_gpt_final_fallback: true, deterministic_policy_kernel: true, model_output_contract_validation: true, authenticated_fallback_selftests: true } });
  }
