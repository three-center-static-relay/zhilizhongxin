...
const REQUIRED_FIELDS=Object.freeze([
  ...
  "verification","license","jurisdiction","first_seen","last_verified"
]);
...
export function makeCapability(input){
  const observedAt=text(input?.freshness?.observed_at,text(input?.health?.checked_at,new Date().toISOString()));
  const lastVerified=validDate(input?.last_verified)?input.last_verified:null;
  const rawUnitCost=input?.cost?.unit_cost;
  const unitCost=rawUnitCost===null||rawUnitCost===undefined||rawUnitCost===""?null:Math.max(0,Number(rawUnitCost)||0);
  return {
    ...
    cost:{class:text(input?.cost?.class,"unknown"),currency:text(input?.cost?.currency,"USD"),unit_cost:unitCost,estimate_status:text(input?.cost?.estimate_status,unitCost===null?"unknown":"declared")},
    ...
    reliability:{score:score(input?.reliability?.score??0),basis:text(input?.reliability?.basis,"unverified-no-bound-runtime-receipt")},
    accuracy:{score:score(input?.accuracy?.score??0),basis:text(input?.accuracy?.basis,"unverified-no-bound-runtime-receipt")},
    ...
    fitness:{
      quality:score(input?.fitness?.quality??input?.accuracy?.score??0),reliability:score(input?.fitness?.reliability??input?.reliability?.score??0),
      cost:score(input?.fitness?.cost??0),latency:score(input?.fitness?.latency??0),security:score(input?.fitness?.security??0),
      adaptability:score(input?.fitness?.adaptability??0),complexity:score(input?.fitness?.complexity??0)
    },
    trust:{level:TRUST_LEVELS.includes(input?.trust?.level)?input.trust.level:"T0",status:text(input?.trust?.status,"quarantined")},
    verification:{status:text(input?.verification?.status,"unverified"),scope:text(input?.verification?.scope,"none"),verified_at:validDate(input?.verification?.verified_at)?input.verification.verified_at:lastVerified,receipt_digest:text(input?.verification?.receipt_digest)||null,sample_size:Math.max(0,Math.trunc(Number(input?.verification?.sample_size)||0))},
    license:text(input?.license,"internal"),jurisdiction:list(input?.jurisdiction).length?list(input.jurisdiction):["global"],
    first_seen:text(input?.first_seen,observedAt),last_verified:lastVerified
  };
}
...
export function validateCapability(capability){
  ...
  const unknownCost=capability.cost?.unit_cost===null&&capability.cost?.estimate_status==="unknown";
  const knownCost=capability.cost?.unit_cost!==null&&finiteBetween(capability.cost?.unit_cost,0,Number.MAX_SAFE_INTEGER)&&text(capability.cost?.estimate_status);
  if(!plain(capability.cost)||!text(capability.cost?.class)||!text(capability.cost?.currency)||(!unknownCost&&!knownCost))errors.push("INVALID_COST");
  ...
  if(!plain(capability.verification)||!text(capability.verification?.status)||!text(capability.verification?.scope)||!nullableDate(capability.verification?.verified_at)||!(capability.verification?.receipt_digest===null||text(capability.verification?.receipt_digest))||!Number.isInteger(capability.verification?.sample_size)||capability.verification.sample_size<0)errors.push("INVALID_VERIFICATION");
  if(capability.verification?.status==="verified"&&(!validDate(capability.verification?.verified_at)||!/^[a-f0-9]{64}$/i.test(String(capability.verification?.receipt_digest||""))||capability.verification?.sample_size<1))errors.push("VERIFIED_RECEIPT_REQUIRED");
  if(!validDate(capability.first_seen)||!nullableDate(capability.last_verified))errors.push("INVALID_VERIFICATION_DATE");
