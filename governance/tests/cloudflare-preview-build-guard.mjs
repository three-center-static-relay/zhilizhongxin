...
globalThis.fetch=async(input,init={})=>{
  ...
  if(method==="GET"&&m){const tag=decodeURIComponent(m[1]);return Response.json({success:true,result:[{trigger_uuid:`preview-${tag}`,deploy_command:"npx wrangler versions upload",branch_includes:["*"],branch_excludes:["main"]}]});}
