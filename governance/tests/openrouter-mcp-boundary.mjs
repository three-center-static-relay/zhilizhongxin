...
assert.equal(policy.version,"openrouter-mcp-governance-layer-v2-20260818");
...
assert.equal(policy.hard_boundaries.fallback,"continue-production-on-openrouter-via-cloudflare-ai-gateway-without-mcp");
...
assert.equal(policy.production_contract.expert_transport,"cloudflare-ai-gateway-openrouter-rest");
assert.equal(policy.production_contract.catalog_endpoint,"https://openrouter.ai/api/v1/models");
assert.equal(policy.production_contract.chat_endpoint_template,"https://gateway.ai.cloudflare.com/v1/{account_id}/{gateway_id}/openrouter/chat/completions");
assert.equal(policy.production_contract.authenticated_gateway,true);
assert.equal(policy.production_contract.cache,false);
assert.equal(policy.production_contract.request_logging,false);
assert.equal(policy.production_contract.gateway_retries,0);
