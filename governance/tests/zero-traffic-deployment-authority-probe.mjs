...
function uploadCandidate(){const dir=mkdtempSync(join(tmpdir(),"gov-zero-traffic-")),output=join(dir,"wrangler.ndjson"),tag=`gov-zero-${sha.slice(0,12)}`;try{const env={...process.env,WRANGLER_OUTPUT_FILE_PATH:output};const r=run(["versions","upload","--name",worker,"--tag",tag,"--message","Zero-traffic deployment authority probe"],env);assert.equal(r.status,0,"VERSION_UPLOAD_FAILED");const lines=readFileSync(output,"utf8").split(/\r?\n/).filter(Boolean).map(line=>parseJson(line,"WRANGLER_OUTPUT"));const event=[...lines].reverse().find(x=>x?.type==="version-upload"&&String(x?.version_id||"").trim());assert.ok(event?.version_id,"UPLOADED_VERSION_ID_NOT_CAPTURED");return String(event.version_id)}finally{rmSync(dir,{recursive:true,force:true})}}
function deploySplit(candidate,active){return run(["versions","deploy",`${candidate}@0%`,`${active}@100%`,"--name",worker,"--message","Zero-traffic deployment authority probe","-y"])}
function restore(active){return run(["versions","deploy",`${active}@100%`,"--name",worker,"--message","Restore after zero-traffic deployment authority probe","-y"])}
...
try{
  ...
  console.log(JSON.stringify({ok:true,suite:"governance-zero-traffic-deployment-authority",version_upload:true,deployment_write:true,candidate_percentage:0,active_percentage:100,production_traffic_changed:false,secrets_redacted:true}));
} finally {
  ...
  if(staged&&restoreAttempted)console.log(JSON.stringify({ok:true,event:"ZERO_TRAFFIC_DEPLOYMENT_RESTORED",single_active_version:true,active_percentage:100,production_traffic_changed:false,secrets_redacted:true}));
}
