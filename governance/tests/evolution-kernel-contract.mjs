...
const canonicalBlocked=await compileTaskPlan({...task,task_id:"adaptive-contract-3",required_capabilities:["compute.nonexistent-capability"]},manifests);
assert.equal(canonicalBlocked.ok,false);
assert.equal(canonicalBlocked.status,"BLOCKED");
assert.deepEqual(canonicalBlocked.unresolved,["compute.nonexistent-capability"]);
assert.equal(canonicalBlocked.graph.nodes.length,0);
assert.equal(canonicalBlocked.execution_started,false);
assert.equal(canonicalBlocked.production_mutation,false);
const semanticFallback=await compileTaskPlan({...task,task_id:"adaptive-contract-4",required_capabilities:["simulation"]},manifests);
assert.equal(semanticFallback.ok,true);
assert.equal(semanticFallback.graph.nodes[0].capability_id,"compute.simulation");
