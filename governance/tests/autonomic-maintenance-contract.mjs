...
import {MAINTENANCE_MODEL_POLICY,chooseMaintenanceBrain,chooseReviewer,repairExecutionPolicy,leaderboardCandidatePolicy} from "../src/maintenance-model-policy.js";
import {classifyIncident,healthScore,planAutonomicRepair,validateReleaseGate} from "../src/autonomic-maintenance.js";

assert.equal(MAINTENANCE_MODEL_POLICY.strategy,"cloudflare-free-stable-pool-with-dynamic-health-routing");
assert.equal(MAINTENANCE_MODEL_POLICY.discovery_authority,"leaderboards-discovery-only-not-runtime-authority");
assert.equal(MAINTENANCE_MODEL_POLICY.fixed_deepseek,false);
...
assert.equal(leaderboardCandidatePolicy({model:"candidate",health_pass:true,contract_pass:true,free_or_budget_ok:true}).direct_production_selection,false);
