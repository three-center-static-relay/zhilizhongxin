...
globalThis.fetch=async (input,init={})=>{
  ...
  if(method==="GET"&&m){const tag=decodeURIComponent(m[1]);assert.ok(byTag.has(tag));return Response.json({success:true,result:[{trigger_uuid:`preview-${tag}`,deploy_command:"npm run cf:build && wrangler versions upload",branch_includes:["*"],branch_excludes:["main"]}]});}
