...
function wrangler(v,args,{encoding="utf8"}={}){
  const r=spawnSync("npx",["--yes",`wrangler@${v}`,...args],{cwd:process.cwd(),env:process.env,encoding,maxBuffer:8*1024*1024});
  if(r.error)throw r.error;
  if(r.status!==0){const text=`${r.stderr||""}\n${r.stdout||""}`;const e=new Error(`WRANGLER_FAILED:${safe(text||r.status)}`);e.exitCode=r.status||1;e.stdout=r.stdout;e.stderr=r.stderr;throw e}
  return r;
}
...
function main(){
  ...
  emit({ok:true,event:"GOVERNANCE_ATOMIC_PRODUCTION_START",commit_sha:sha,primary_model:PRIMARY_MODEL,transport:"direct-workflow-binding",model_sources:["workers-ai","openrouter","huggingface"],free_first:true,auto_paid_budget_usd:0});
  run("npm",["run","cf:build"],{stdio:"inherit"});
  const check=wrangler(v,["deploy","--config","wrangler.jsonc","--dry-run"],{encoding:"utf8"});
  if(check.stdout)process.stdout.write(check.stdout);if(check.stderr)process.stderr.write(check.stderr);
  const deploy=wrangler(v,["deploy","--config","wrangler.jsonc"],{encoding:"utf8"});
  if(deploy.stdout)process.stdout.write(deploy.stdout);if(deploy.stderr)process.stderr.write(deploy.stderr);
  emit({ok:true,event:"GOVERNANCE_ATOMIC_PRODUCTION_PASS",commit_sha:sha,workflow_binding:true,analytics_binding:true,workers_ai_binding:true,primary_model:PRIMARY_MODEL,transport:"direct-workflow-binding",active_percentage:100,active_percentage_authority:"wrangler-deploy-success",rollback_available:true,model_sources:["workers-ai","openrouter","huggingface"],free_first:true,auto_paid_budget_usd:0});
}
