...
console.log(JSON.stringify({event:"MAINTENANCE_RESCUE_BEGIN",branch:EXPECTED_BRANCH,commit_sha:process.env.WORKERS_CI_COMMIT_SHA||null}));
...
if(run.error||run.status!==0)fail("MAINTENANCE_RESCUE_FAILED",{status:run.status??null,error:run.error?.message||null});
console.log(JSON.stringify({ok:true,code:"MAINTENANCE_RESCUE_COMPLETED",commit_sha:process.env.WORKERS_CI_COMMIT_SHA||null}));
