...
async function runOnce(env,attempt){
  ...
  if(!lr?.ok||la?.ok!==true)return{ok:false,http_status:lr?.status||502,selftest:"autonomic-expert-e2e-v20",attempt,started_at,deploy_marker:DEPLOY_MARKER,stage:"langgraph-validation",langgraph:la,secrets_redacted:true};
  if(!env.EXPERT_CENTER?.fetch)return{ok:false,http_status:503,selftest:"autonomic-expert-e2e-v20",attempt,started_at,deploy_marker:DEPLOY_MARKER,stage:"expert-execution",error:"EXPERT_SERVICE_BINDING_UNAVAILABLE",secrets_redacted:true};
  const started=Date.now();
  const er=await env.EXPERT_CENTER.fetch(new Request("https://expert.internal/v1/run",{method:"POST",headers:{"content-type":"application/json",accept:"application/json"},body:JSON.stringify({task_id:expertTaskId,prompt:PROMPT,task_domain:"business",task_type:"comparison",complexity:"high",reasoning_depth:"deep",cost_mode:"quality-first",model_count:2,rounds:1,timeout_seconds:300,tools:false,web:false})}));
  const expert=await bounded(er).catch(error=>({ok:false,error:String(error?.message||error)})),ok=er.ok&&expert?.ok===true;
  return{ok,http_status:ok?200:(er.status||502),selftest:"autonomic-expert-e2e-v20",attempt,started_at,finished_at:new Date().toISOString(),deploy_marker:DEPLOY_MARKER,expert_task_id:expertTaskId,stage:ok?"completed":"expert-execution",elapsed_ms:Date.now()-started,recovery_profile:{requested_model_count:2,rounds:1,effective_model_timeout_ms:30000,requested_cost_mode:"quality-first",semantic_prompt_unmodified:true,model_provider_dynamic:true},langgraph:{ok:true,status:la?.status||null,runtime:la?.runtime||null,runtime_host:la?.runtime_host||null,control_plane:la?.control_plane||null,planner:la?.planner||null,brain_can_command:la?.brain_can_command===true,execution_mode:la?.execution_mode||null,plan_digest:la?.plan_digest||null,plan_path:la?.plan_path||null,planned_centers:la?.planned_centers||[]},expert_http_status:er.status,expert,tools_used:false,web_used:false,production_mutation:false,secrets_redacted:true};
}
...
export async function handleAutonomicExpertE2EV20(request,env){
  ...
  if(operation==="status")return json({ok:true,selftest:"autonomic-expert-e2e-v20",deploy_marker:DEPLOY_MARKER,connected_client_required:true,expert:await expertContext(env),expires_at:new Date(EXPIRES_AT).toISOString(),secrets_redacted:true});
  ...
  catch(error){return json({ok:false,http_status:error?.status||500,selftest:"autonomic-expert-e2e-v20",deploy_marker:DEPLOY_MARKER,stage:"canary",error:String(error?.message||error).slice(0,180),secrets_redacted:true},error?.status||500)}
}
