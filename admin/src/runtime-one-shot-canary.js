...
async function runOnce(input,env,requestId){
  ...
  return{ok,http_status:ok?200:(expertResponse.status||502),selftest:"runtime-fuzhou-work-choice-v3",stage:ok?"completed":"expert-execution",langgraph:{ok:true,status:la?.status||null,runtime:la?.runtime||null,runtime_host:la?.runtime_host||null,control_plane:la?.control_plane||null,planner:la?.planner||null,brain_can_command:la?.brain_can_command===true,execution_mode:la?.execution_mode||null},policy:{cost_control:"soft-price-performance",length_control:"soft-adaptive",token_cap:false,tools:false,web:false},expert_http_status:expertResponse.status,expert,tools_used:false,web_used:false,production_mutation:false,secrets_redacted:true};
}

export async function handleRuntimeOneShotCanary(request,env){
  const url=new URL(request.url),base="/__runtime-canary/fuzhou-work-choice";
  ...
  const requestId=String(input?.request_id||"fuzhou-effect-v3").replace(/[^0-9A-Za-z._:-]/g,"_").slice(0,160),taskKey=`runtime-canary-${requestId}`,owner=`runtime-canary:${requestId}`;
