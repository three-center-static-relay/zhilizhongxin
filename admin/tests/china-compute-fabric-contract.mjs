...
p=planChinaFabric({capability:"free-cpu-validation"},status);
assert.equal(p.provider,"modelscope");assert.equal(p.execution_allowed,true);assert.equal(p.cost_class,"free-only");assert.equal(p.validation_only,true);

p=planChinaFabric({capability:"free-cpu"},status);
assert.equal(p.ok,false);assert.equal(p.error,"UNSUPPORTED_CAPABILITY");
...
p=planChinaFabric({capability:"free-cpu-validation"},down);
...
for(const literal of [
  'automatic_business_provider_set:["tencent"]',
  'validation_only_provider_set:["modelscope"]',
  'explicit_metered_provider_set:["baidu","huawei"]',
  'explicit_paid_provider_set:["aliyun"]',
  'generic_business_task_adapter:false',
  'paid_fallback:false',
