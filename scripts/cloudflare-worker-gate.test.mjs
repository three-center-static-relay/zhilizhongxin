...
for(const scope of["admin","governance","maintenance"]){
  assert.deepEqual(wranglerCommand(scope,"preview","4.123.0"),["--yes","wrangler@4.123.0","deploy","--dry-run"]);
}
...
console.log(JSON.stringify({ok:true,suite:"cloudflare-worker-gate-contract",fail_closed_context:true,exact_wrangler_pin:true,per_worker_path_isolation:true,build_side_effect_post_hooks_disabled:true,all_preview_scopes_compile_dry_run_only:true,maintenance_versions_upload_disabled_due_durable_object_exports:true,l2_requires_explicit_trigger_path:true,l2_only_runs_in_maintenance_preview:true}));
