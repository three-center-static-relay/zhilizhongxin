...
// Shared-build synchronization trigger for Tencent 78d receipt-contract classification; behavior unchanged.
...
export async function fetchAttestation(base,expectedCommit){
  ...
  try{
    const response=await fetch(`${base.replace(/\/+$/,'')}/_internal/tencent-production-attestation`,{method:"GET",headers:{accept:"application/json"},signal:controller.signal});
...
function adminBase(repoRoot){const spec=JSON.parse(readFileSync(resolve(repoRoot,"admin/openapi.json"),"utf8"));const base=String(spec?.servers?.[0]?.url||"").replace(/\/+$/,'');if(!base)throw new Error("ADMIN_OPENAPI_SERVER_REQUIRED");return base}
function parentSha(repoRoot,sha){const result=spawnSync("git",["cat-file","-p",sha],{cwd:repoRoot,encoding:"utf8",env:process.env});if(result.error)throw result.error;if(result.status!==0)throw new Error("GIT_PARENT_LOOKUP_FAILED");const parent=result.stdout.match(/^parent ([a-f0-9]{40,64})$/im)?.[1]||"";if(!SHA.test(parent))throw new Error("VALID_PARENT_COMMIT_REQUIRED");return parent}

async function main(){
  const mode=process.argv[2]||"direct";const repoRoot=process.cwd().endsWith("/admin")?resolve(process.cwd(),".."):process.cwd();
  if(mode==="preview-marker"){
    const marker=resolve(repoRoot,"admin/canary/tencent-production-attestation-verify.txt");
    if(!existsSync(marker)){console.log(JSON.stringify({ok:true,skipped:true,suite:"tencent-production-attestation",reason:"MARKER_ABSENT"}));return}
    if(process.env.WORKERS_CI!=="1"||process.env.WORKERS_CI_BRANCH==="main")throw new Error("ATTESTATION_PREVIEW_CONTEXT_REQUIRED");
    const current=String(process.env.WORKERS_CI_COMMIT_SHA||"");if(!SHA.test(current))throw new Error("VALID_WORKERS_CI_COMMIT_SHA_REQUIRED");
    const expected=parentSha(repoRoot,current),base=adminBase(repoRoot),body=await fetchAttestation(base,expected);
    console.log(JSON.stringify({ok:true,skipped:false,suite:"tencent-production-attestation",attested_commit:body.attested_commit,runtime_e2e:true,deploy_probe_active:false}));return;
  }
  const base=String(process.argv[2]||""),expected=String(process.argv[3]||""),body=await fetchAttestation(base,expected);
