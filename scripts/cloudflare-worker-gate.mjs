...
export function main(argv=process.argv.slice(2),env=process.env){
  ...
  try{
    ...
    emit({ok:true,skipped:false,code:"CF_PATH_SCOPE_ALLOWED",scope,mode,branch:context.branch,commit_sha:context.sha,parent_sha:parentSha,history_deepened:historyDeepened,relevant_paths:relevant,wrangler_version:wranglerVersion,preview_semantics:mode==="preview"?"compile-dry-run-only":"production-deploy",worker_deployment_mutation_allowed:mode==="deploy",l2_requested:l2,l2_execution_mode:l2?"maintenance-local-candidate-remote-bindings":null,post_allow_script:null});
    ...
    if(l2){
      emit({event:"L2_EXPERT_ROUTE_ACCEPTANCE_BEGIN",commit_sha:context.sha,execution_mode:"maintenance-local-candidate-remote-bindings",worker_deployment_mutation:false,dynamic_route_acceptance:true});
      run(process.execPath,[resolve(process.cwd(),L2_SCRIPT)],{cwd:process.cwd(),stdio:"inherit"});
      emit({event:"L2_EXPERT_ROUTE_ACCEPTANCE_COMPLETED",commit_sha:context.sha,execution_mode:"maintenance-local-candidate-remote-bindings",worker_deployment_mutation:false,dynamic_route_acceptance:true});
    }
    return 0;
