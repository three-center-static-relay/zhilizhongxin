...
export function validatePostAllowScript(scope,mode,requested=null){if(!requested)return null;if(!SCOPES.has(scope)||!MODES.has(mode))throw new Error("POST_ALLOW_SCRIPT_NOT_ALLOWED");throw new Error("POST_ALLOW_SCRIPT_NOT_ALLOWED")}
...
export function wranglerCommand(scope,mode,wranglerVersion,context){
  validateWranglerVersion(wranglerVersion);if(!SCOPES.has(scope))throw new Error(`UNSUPPORTED_SCOPE:${scope||"missing"}`);
  if(mode==="preview"){
...
export function main(argv=process.argv.slice(2),env=process.env){
  ...
  try{
    ...
    const repoRoot=repositoryRoot(),{parentSha,historyDeepened}=diffContext(repoRoot,context.sha,context.branch),changed=changedPaths(repoRoot,parentSha,context.sha),relevant=relevantPaths(scope,changed),l2=shouldRunL2(scope,mode,changed);
    if(relevant.length===0){emit({ok:true,skipped:true,code:"CF_PATH_SCOPE_SKIPPED",scope,mode,branch:context.branch,commit_sha:context.sha,parent_sha:parentSha,history_deepened:historyDeepened,changed_path_count:changed.length,post_allow_executed:false,l2_executed:false});return 0}
    const{wranglerVersion}=packageContract(scope),previewSemantics=mode==="preview"?(scope==="admin"||scope==="maintenance"?"version-upload-no-production-deploy":"compile-dry-run-only"):"production-deploy";
    emit({ok:true,skipped:false,code:"CF_PATH_SCOPE_ALLOWED",scope,mode,branch:context.branch,commit_sha:context.sha,parent_sha:parentSha,history_deepened:historyDeepened,relevant_paths:relevant,wrangler_version:wranglerVersion,preview_semantics:previewSemantics,candidate_tag:mode==="preview"&&(scope==="admin"||scope==="maintenance")?context.sha.slice(0,12):null,post_allow_script:null,l2_requested:l2});
    ...
    if(l2){
      emit({event:"L2_EXPERT_ROUTE_ACCEPTANCE_BEGIN",scope,commit_sha:context.sha,trigger_path:L2_TRIGGER_PATH});
      run(process.execPath,[resolve(process.cwd(),L2_SCRIPT)],{cwd:process.cwd(),stdio:"inherit"});
      emit({event:"L2_EXPERT_ROUTE_ACCEPTANCE_COMPLETED",scope,commit_sha:context.sha,trigger_path:L2_TRIGGER_PATH});
    }
    return 0;
