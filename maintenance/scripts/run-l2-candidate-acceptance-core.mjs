...
function compatibilityExpertBinding(env,acceptancePhase){
  return {
    async fetch(request){
      const u=new URL(request.url);
      if(u.pathname==="/v1/admin/context")return Response.json({ok:true,active_task:null,compatibility_adapter:true});
      if(u.pathname==="/v1/selftest"){
        if(acceptancePhase==="predeploy-control-plane")return Response.json({ok:true,company_diverse:true,models:[],runtime_selftest_skipped:true,compatibility_adapter:true});
        const taskId="l2-postdeploy-"+crypto.randomUUID();
        const response=await env.EXPERT_CENTER.fetch(new Request("https://expert.internal/v1/run",{method:"POST",headers:{accept:"application/json","content-type":"application/json","x-three-center-selftest":"1"},body:JSON.stringify({task_id:taskId,prompt:"L2 production route self-test. Return a concise deterministic response.",model_count:2,timeout_seconds:120})}));
        const body=await response.json().catch(()=>({}));
        return Response.json({ok:response.ok&&body?.ok===true&&body?.company_diverse===true,company_diverse:body?.company_diverse===true,models:Array.isArray(body?.models)?body.models:[],upstream_http_status:response.status,upstream_error:body?.error||null,compatibility_adapter:true},{status:response.ok?200:response.status||502});
      }
      return env.EXPERT_CENTER.fetch(request);
    }
  };
}
export default{async fetch(request,env){const u=new URL(request.url);if(request.method==="GET"&&u.pathname==="/ready")return Response.json({ok:true});if(request.method!=="POST"||u.pathname!=="/run")return Response.json({ok:false,error:"NOT_FOUND"},{status:404});try{const acceptancePhase=${JSON.stringify(acceptancePhase)};const expertBinding=compatibilityExpertBinding(env,acceptancePhase);const result=await refreshExpertRoute(env,{previous:null,expertBinding});const routes=Array.isArray(result?.route_family)?result.route_family:[],lanes=Array.isArray(result?.company_lanes)?result.company_lanes:[],companies=lanes.map(x=>String(x?.company||"")).filter(Boolean);const routeOk=result?.ok===true&&result?.status==="active"&&routes.length===8&&lanes.length===8&&new Set(companies).size===8;const runtimeOk=acceptancePhase==="predeploy-control-plane"||result?.selftest?.ok===true&&result?.selftest?.company_diverse===true;const baseOk=routeOk&&runtimeOk;if(!baseOk)return Response.json({ok:false,error:"L2_ROUTE_ACCEPTANCE_CONTRACT_FAILED",acceptance_phase:acceptancePhase,result,secrets_redacted:true},{status:502});const rollback=await rehearse(env,result);const ok=rollback.ok===true;return Response.json({ok,result,acceptance_phase:acceptancePhase,runtime_selftest_required:acceptancePhase==="postdeploy-runtime",runtime_selftest_ok:acceptancePhase==="postdeploy-runtime"?result?.selftest?.ok===true:null,rollback_rehearsal:rollback,secrets_redacted:true},{status:ok?200:502})}catch(error){return Response.json({ok:false,error:String(error?.message||error),details:error?.details||null,secrets_redacted:true},{status:error?.status||502})}}};
`);
  ...
  try{
    mark("local-candidate-start",{acceptance_phase:acceptancePhase,local_worker_execution:true,remote_expert_binding:true,worker_deployment_mutation:false,dynamic_route_mutation:true});
