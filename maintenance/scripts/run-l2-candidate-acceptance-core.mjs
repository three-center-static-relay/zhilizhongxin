...
function brokerizeManager(source){
  ...
  if(!control?.request||!gatewayId||!routeFamily)throw fail("EXPERT_ROUTE_CONTROL_PLANE_NOT_CONFIGURED",503,{broker_configured:Boolean(control?.request),gateway_id_configured:Boolean(gatewayId),route_family_configured:Boolean(routeFamily)});
  return {gatewayId,routeFamily,control};
}`;
...
async function main(){
  ...
  const routeFamily=String(process.env.AI_GATEWAY_ROUTE_FAMILY||"expert-panel").trim();
...
export default{async fetch(request,env){const u=new URL(request.url);if(request.method==="GET"&&u.pathname==="/ready")return Response.json({ok:true});if(request.method!=="POST"||u.pathname!=="/run")return Response.json({ok:false,error:"NOT_FOUND"},{status:404});try{const acceptancePhase=${JSON.stringify(acceptancePhase)};const expertBinding=compatibilityExpertBinding(env,acceptancePhase);const result=await refreshExpertRoute(env,{previous:null,expertBinding});const routes=Array.isArray(result?.route_family)?result.route_family:[],lanes=Array.isArray(result?.company_lanes)?result.company_lanes:[],companies=lanes.map(x=>String(x?.company||"")).filter(Boolean);const noLegacy=routes.length>0&&routes.every(x=>x?.route_name!=="expert-panel-v1"&&x?.legacy!==true);const routeOk=result?.ok===true&&result?.status==="active"&&noLegacy&&lanes.length>=2&&lanes.length<=8&&new Set(companies).size===lanes.length;const runtimeOk=acceptancePhase==="predeploy-control-plane"||result?.selftest?.ok===true&&result?.selftest?.company_diverse===true;if(!(routeOk&&runtimeOk))return Response.json({ok:false,error:"L2_ROUTE_ACCEPTANCE_CONTRACT_FAILED",acceptance_phase:acceptancePhase,result,secrets_redacted:true},{status:502});const rollback=await rehearse(env,result);const ok=rollback.ok===true;return Response.json({ok,result,acceptance_phase:acceptancePhase,runtime_selftest_required:acceptancePhase==="postdeploy-runtime",runtime_selftest_ok:acceptancePhase==="postdeploy-runtime"?result?.selftest?.ok===true:null,rollback_rehearsal:rollback,secrets_redacted:true},{status:ok?200:502})}catch(error){return Response.json({ok:false,error:String(error?.message||error),details:error?.details||null,secrets_redacted:true},{status:error?.status||502})}}};
`);
  writeFileSync(resolve(dir,"wrangler.jsonc"),JSON.stringify({name:"l2-local-route-candidate-driver",main:"driver.mjs",compatibility_date:"2026-08-20",compatibility_flags:["nodejs_compat"],vars:{AI_GATEWAY_ID:gatewayId,AI_GATEWAY_ROUTE_FAMILY:routeFamily},services:[{binding:"EXPERT_CENTER",service:"expert-worker",remote:true},{binding:"AI_GATEWAY_CONTROL",service:"admin-worker",entrypoint:"AIGatewayControl",remote:true}]},null,2));
  let child=null,stderr="";
  try{
    mark("local-candidate-start",{acceptance_phase:acceptancePhase,local_worker_execution:true,remote_expert_binding:true,remote_ai_gateway_control:true,worker_deployment_mutation:false,dynamic_route_mutation:true,legacy_route_removed:true});
