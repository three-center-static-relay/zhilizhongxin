...
const APPROVED_MODEL_SOURCES=["workers-ai","openrouter","huggingface"];
...
function deployCandidate(version,probe){const args=["--yes",`wrangler@${version}`,"deploy","--var",`MAINTENANCE_RUNTIME_E2E_PROBE:${probe}`,"--var","MAINTENANCE_AI_GATEWAY_WRITE_SCOPE_PROBE:1","--var","EXPERT_ROUTE_AUTOMATION_ENABLED:false","--var","EXPERT_MODEL_SOURCE_POLICY:strict-three-source-free-first","--var","EXPERT_MODEL_SOURCE_CLASSES:workers-ai,openrouter,huggingface"];const r=run("npx",args,{encoding:"utf8"}),output=`${r.stdout||""}\n${r.stderr||""}`,url=parseWorkersDevUrl(output);emit({ok:true,event:"MAINTENANCE_WRITE_SCOPE_CANDIDATE_DEPLOYED",worker_host:new URL(url).host,probe_persisted:true,write_scope_probe_enabled:true,route_automation_enabled:false,production_dynamic_route_mutation:false,model_invocation:false,model_source_classes:APPROVED_MODEL_SOURCES});return{url}}
function verifyWriteScope(url,probe){const script=resolve(process.cwd(),"scripts/runtime-ai-gateway-write-scope-e2e.mjs"),r=spawnSync(process.execPath,[script,url],{cwd:process.cwd(),encoding:"utf8",env:{...process.env,MAINTENANCE_E2E_PROBE_TOKEN:probe},maxBuffer:8*1024*1024});if(r.stdout)process.stdout.write(r.stdout);if(r.stderr)process.stderr.write(r.stderr);if(r.error){const e=new Error("WRITE_SCOPE_E2E_SPAWN_ERROR");e.exitCode=1;throw e}if(r.status!==0){const m=`${r.stderr||""}\n${r.stdout||""}`.match(/AI_GATEWAY_WRITE_SCOPE_E2E_FAILED:([^\r\n]+)/);const e=new Error(`WRITE_SCOPE_E2E_FAILED:${safeCode(m?.[1]||`EXIT_${r.status??1}`)}`);e.exitCode=1;throw e}emit({ok:true,event:"MAINTENANCE_WRITE_SCOPE_E2E_PASS",worker_host:new URL(url).host,workers_dev_https_reachable:true,probe_authenticated:true,ai_gateway_routes_readable:true,ai_gateway_write_authorized:true,resource_mutated:false,production_route_changed:false,model_invocation:false,model_source_classes:APPROVED_MODEL_SOURCES})}
function main(){
  ...
  emit({ok:true,event:"MAINTENANCE_WRITE_SCOPE_STAGE_START",commit_sha:sha,policy_owner:"admin-worker",route_execution_owner:"maintenance-worker",credential_custodian:"maintenance-worker",cloudflare_credentials_local:true,model_id_pinning:false,future_models_auto_discover:true,model_source_classes:APPROVED_MODEL_SOURCES,route_automation_enabled:false,production_dynamic_route_mutation:false,model_invocation:false});
