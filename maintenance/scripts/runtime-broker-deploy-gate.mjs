...
function deployCandidate(version,probe){const args=["--yes",`wrangler@${version}`,"deploy","--var",`MAINTENANCE_RUNTIME_E2E_PROBE:${probe}`,"--var","MAINTENANCE_MODEL_UNIVERSE_PROBE:1"];const r=run("npx",args,{encoding:"utf8"}),output=`${r.stdout||""}\n${r.stderr||""}`,url=parseWorkersDevUrl(output);emit({ok:true,event:"MAINTENANCE_HTTP_PROBE_CANDIDATE_DEPLOYED",worker_host:new URL(url).host,probe_persisted:true,model_universe_probe_enabled:true,route_automation_enabled:false,production_dynamic_route_mutation:false,model_invocation:false});return{url}}
function verifyHttp(url,probe){const script=resolve(process.cwd(),"scripts/runtime-broker-postdeploy-e2e.mjs"),r=spawnSync(process.execPath,[script,url],{cwd:process.cwd(),encoding:"utf8",env:{...process.env,MAINTENANCE_E2E_PROBE_TOKEN:probe},maxBuffer:8*1024*1024});if(r.stdout)process.stdout.write(r.stdout);if(r.stderr)process.stderr.write(r.stderr);if(r.error){const e=new Error("HTTP_PROBE_E2E_SPAWN_ERROR");e.exitCode=1;throw e}if(r.status!==0){const m=`${r.stderr||""}\n${r.stdout||""}`.match(/MAINTENANCE_POSTDEPLOY_E2E_FAILED:([^\r\n]+)/);const e=new Error(`HTTP_PROBE_E2E_FAILED:${safeCode(m?.[1]||`EXIT_${r.status??1}`)}`);e.exitCode=1;throw e}emit({ok:true,event:"MAINTENANCE_HTTP_PROBE_E2E_PASS",worker_host:new URL(url).host,probe_authenticated:true,workers_dev_https_reachable:true,model_universe_e2e_invoked:false,production_dynamic_route_mutation:false,model_invocation:false})}
function main(){
  ...
  try{const candidate=deployCandidate(version,probe);verifyHttp(candidate.url,probe)}catch(error){primaryError=error}
  const restore=spawnSync(process.execPath,[sharedGate,"maintenance","deploy"],{cwd:process.cwd(),env:process.env,stdio:"inherit"});
  if(restore.error||restore.status!==0){const e=new Error("CLEAN_RESTORE_AFTER_HTTP_PROBE_FAILED");e.exitCode=restore.status||1;throw e}
  if(primaryError)throw primaryError;
  emit({ok:true,event:"MAINTENANCE_HTTP_PROBE_STAGE_COMPLETE",commit_sha:sha,probe_persisted:false,model_universe_probe_persisted:false,workers_dev_https_reachable:true,probe_authenticated:true,production_attestation:true,policy_owner:"admin-worker",route_execution_owner:"maintenance-worker",credential_custodian:"maintenance-worker",model_id_pinning:false,future_models_auto_discover:true,route_automation_enabled:false,production_dynamic_route_mutation:false,model_invocation:false});
}
const invoked=process.argv[1]?pathToFileURL(resolve(process.argv[1])).href:"";
if(import.meta.url===invoked){try{main()}catch(error){emit({ok:false,event:"MAINTENANCE_HTTP_PROBE_STAGE_FAIL",code:safeCode(error?.message||error)},process.stderr);process.exitCode=Number.isInteger(error?.exitCode)?error.exitCode:1}}
