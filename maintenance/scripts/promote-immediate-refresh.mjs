...
const r=spawnSync("npx",["--yes","wrangler@4.123.0","deploy"],{stdio:"inherit",env:process.env,encoding:"utf8"});
if(r.error||r.status!==0)fail("IMMEDIATE_REFRESH_DEPLOY_FAILED");
console.log(JSON.stringify({ok:true,code:"IMMEDIATE_REFRESH_DEPLOYED",branch:EXPECTED_BRANCH,commit_sha:process.env.WORKERS_CI_COMMIT_SHA}));
