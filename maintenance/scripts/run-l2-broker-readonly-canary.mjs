...
async function main(){
  ...
  emit({event:"L2_BROKER_READONLY_CANARY_START",phase:"start",commit_sha:commit||null,max_attempts:MAX_ATTEMPTS,direct_wrangler_binary:true,single_driver_lifecycle:true,production_worker_traffic_changed:false,dynamic_route_mutation:false});
  let child=null,last={error_code:"NOT_ATTEMPTED",broker_rpc:false,routes_readable:false,attempt:0};
  try{
    child=spawn(wranglerBin,["dev","--config","wrangler.jsonc","--ip","127.0.0.1","--port",String(PORT)],{cwd:dir,env:{...process.env,CI:"1",NO_COLOR:"1"},detached:process.platform!=="win32",stdio:["ignore","ignore","ignore"]});
    await waitReady(child);emit({event:"L2_BROKER_READONLY_DRIVER_READY",phase:"driver-ready",broker_rpc:false,routes_readable:false});
    for(let attemptNumber=1;attemptNumber<=MAX_ATTEMPTS;attemptNumber++){
      try{
        ...
        if(response.ok&&body?.ok===true){emit({event:"L2_BROKER_READONLY_CANARY_PASS",phase:"pass",attempt:attemptNumber,error_code:null,broker_rpc:true,routes_readable:true,route_count:body?.route_count??null,dynamic_route_mutation:false,production_worker_traffic_changed:false});return}
        last={error_code:String(body?.error_code||`BROKER_READ_HTTP_${response.status}`),broker_rpc:body?.broker_rpc===true,routes_readable:body?.routes_readable===true,attempt:attemptNumber};
      }catch(error){last={error_code:classify(error?.message||error),broker_rpc:false,routes_readable:false,attempt:attemptNumber}}
      emit({event:"L2_BROKER_READONLY_CANARY_RETRY",phase:"broker-read-retry",attempt:attemptNumber,error_code:last.error_code,broker_rpc:last.broker_rpc,routes_readable:last.routes_readable,dynamic_route_mutation:false});
