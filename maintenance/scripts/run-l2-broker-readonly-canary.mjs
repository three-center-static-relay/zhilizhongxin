...
function stop(child){if(!child||child.exitCode!==null)return;try{child.kill("SIGTERM")}catch{}setTimeout(()=>{if(child.exitCode===null)try{child.kill("SIGKILL")}catch{}},1200).unref?.()}
...
async function attempt(dir){
  let child=null;
  try{
    child=spawn("npx",["--yes",`wrangler@${WRANGLER}`,"dev","--config","wrangler.jsonc","--ip","127.0.0.1","--port",String(PORT)],{cwd:dir,env:{...process.env,CI:"1",NO_COLOR:"1"},stdio:["ignore","ignore","ignore"]});
    await waitReady(child);
    const {response,body}=await requestLocal("/run",{method:"POST",headers:{accept:"application/json"}},REQUEST_TIMEOUT_MS);
    if(response.ok&&body?.ok===true)return{ok:true,route_count:body?.route_count??null};
    return{ok:false,error:String(body?.error||`BROKER_READONLY_HTTP_${response.status}`),details:body?.details||null};
  }catch(error){return{ok:false,error:String(error?.message||error)}}
  finally{stop(child);await sleep(1500)}
}
...
async function main(){
  ...
  console.log(JSON.stringify({event:"L2_BROKER_READONLY_CANARY_START",commit_sha:commit||null,max_attempts:MAX_ATTEMPTS,bounded_propagation_wait:true,production_worker_traffic_changed:false,dynamic_route_mutation:false,secrets_redacted:true}));
  let last={ok:false,error:"NOT_ATTEMPTED"};
  try{
    for(let attemptNumber=1;attemptNumber<=MAX_ATTEMPTS;attemptNumber++){
      last=await attempt(dir);
      if(last.ok===true){console.log(JSON.stringify({event:"L2_BROKER_READONLY_CANARY_PASS",ok:true,commit_sha:commit||null,attempt:attemptNumber,broker_rpc:true,routes_readable:true,route_count:last.route_count,dynamic_route_mutation:false,production_worker_traffic_changed:false,secrets_redacted:true}));return}
  ...
  }catch(error){console.error(JSON.stringify({event:"L2_BROKER_READONLY_CANARY_FAIL",error:String(error?.message||error).slice(0,240),details:error?.details||null,attempts:MAX_ATTEMPTS,dynamic_route_mutation:false,production_worker_traffic_changed:false,secrets_redacted:true}));process.exitCode=1}
