...
try{const r=await fetch('https://admin-worker.a15280020511.workers.dev/_internal/tencent-production-attestation',{headers:{accept:'application/json'},signal:c.signal});const b=await r.json().catch(()=>null);const exact=r.status===200&&b?.validation==='FAIL'&&b?.failed_commit===expected&&b?.fail_closed===true&&b?.secret_values_exposed===false;const code=String(b?.failure_code||'');const matched=exact&&!/^HTTP_\d{3}:/i.test(code);console.log(JSON.stringify({suite:'class-nonhttp',matched,secret_values_read:false}));assert.equal(matched,true,'NON_HTTP_REQUIRED')}finally{clearTimeout(timer)}
