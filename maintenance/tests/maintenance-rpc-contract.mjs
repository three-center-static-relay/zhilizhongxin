...
assert.match(source,/\/v1\/control\/expert-route\/refresh/);
assert.match(source,/\/v1\/control\/expert-route\/latest/);
...
assert.match(source,/maintenance_version: versionId\(env\)/);
...
console.log(JSON.stringify({ok:true,suite:"maintenance-rpc-contract",private_named_entrypoint:true,fetch_override_ready:true,rpc_fallback:true,version_receipts:true,workers_dev_disabled:true,build_refresh_removed:true,preview_gate_only:true,wrangler_pinned:true}));
