...
export default{async fetch(request,env){if(!auth(request,env))return json({ok:false,error:"NOT_FOUND"},404);try{const rows=await discoverWorkersAI(env),companies=new Set(rows.map(x=>x.company).filter(Boolean)),ok=rows.length>0&&companies.size>=2;return json({ok,stage:"workers-ai-live-catalog-after-replacement",candidate_count:rows.length,company_count:companies.size,secrets_redacted:true},ok?200:502)}catch(e){return json({ok:false,stage:"workers-ai-live-catalog-after-replacement",error:safe(e?.message||e),secrets_redacted:true},502)}}};
