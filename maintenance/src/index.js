...
async function probeAdminControlRoundtrip(env){
  if(!env.GOVERNANCE_CENTER?.fetch)return{ok:false,binding:false,service_binding:false,transport:"service-binding-fetch",routes_readable:false,error_code:"GOVERNANCE_CENTER_UNBOUND"};
  ...
  try{
    ...
    const serviceBinding=body?.binding===true&&body?.service_binding===true&&body?.transport==="service-binding-fetch";
    const ok=response.status===200&&body?.ok===true&&serviceBinding&&body?.routes_readable===true&&body?.dynamic_route_mutation===false&&body?.expert_called===false&&body?.secrets_redacted===true;
    return{ok,binding:body?.binding===true,service_binding:serviceBinding,transport:String(body?.transport||""),routes_readable:body?.routes_readable===true,error_code:ok?null:String(body?.error_code||`HTTP_${response.status}`).slice(0,120)};
