...
const SERVICE="maintenance-worker",API_VERSION="2026-08-20.credential-read-v1";
const POLICY={scheduled_health_check:true,interval_minutes:15,read_only:true,auto_diagnose:true,consecutive_failure_tracking:true,acceptance_fail_closed_rollback:"admin-worker",automatic_code_upgrade:false,automatic_dependency_upgrade:false,protected_core_auto_upgrade:false,github_write:false,ai_gateway_control_read:true,ai_gateway_control_write:false,cloudflare_credentials_local:true,credential_custodian:"maintenance-worker",credential_scope:"ai_gateway_routes_read_only",control_plane_owner:"admin-worker"};
function json(x,s=200){return Response.json(x,{status:s,headers:{"cache-control":"no-store"}})}
...
async function probeAiGatewayControl(env){
  let timer;
  try{
    const timeout=new Promise((_,reject)=>{timer=setTimeout(()=>reject(new Error("AI_GATEWAY_CREDENTIAL_TIMEOUT")),8000)});
    const payload=await Promise.race([aiGatewayCredentialRoutesList(env),timeout]);
    const success=payload?.success!==false;
    return{ok:success,credential_broker:true,routes_readable:success,credential_source:"maintenance-worker",error_code:success?null:"AI_GATEWAY_CREDENTIAL_READ_FAILED"};
  }catch(error){return{ok:false,credential_broker:false,routes_readable:false,credential_source:"maintenance-worker",error_code:credentialFailureCode(error)}}finally{if(timer)clearTimeout(timer)}
}
async function probeAdminControlRoundtrip(env){
  if(!env.GOVERNANCE_CENTER?.fetch)return{ok:false,binding:false,broker_rpc:false,routes_readable:false,error_code:"GOVERNANCE_CENTER_UNBOUND"};
  const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),12000);
  try{
    const response=await env.GOVERNANCE_CENTER.fetch(new Request("https://governance.internal/_internal/ai-gateway-control-readonly-probe",{method:"GET",headers:{accept:"application/json","x-three-center-selftest":"1"},signal:controller.signal}));
    const body=await response.json().catch(()=>null);
    const ok=response.status===200&&body?.ok===true&&body?.binding===true&&body?.broker_rpc===true&&body?.routes_readable===true&&body?.dynamic_route_mutation===false&&body?.expert_called===false&&body?.secrets_redacted===true;
    return{ok,binding:body?.binding===true,broker_rpc:body?.broker_rpc===true,routes_readable:body?.routes_readable===true,error_code:ok?null:String(body?.error_code||`HTTP_${response.status}`).slice(0,120)};
  }catch(error){return{ok:false,binding:true,broker_rpc:false,routes_readable:false,error_code:String(error?.name==="AbortError"?"ADMIN_CONTROL_ROUNDTRIP_TIMEOUT":"ADMIN_CONTROL_ROUNDTRIP_FAILED")}}finally{clearTimeout(timer)}
}
...
async function runtimeSelftest(req,env){
  ...
  const ok=control.ok&&control.credential_broker&&control.routes_readable&&roundtrip.ok&&roundtrip.binding&&roundtrip.broker_rpc&&roundtrip.routes_readable;
  return json({ok,selftest:"maintenance-ai-gateway-credential-read-v1",service:SERVICE,ai_gateway_control:{credential_broker:control.credential_broker,routes_readable:control.routes_readable,credential_source:control.credential_source,error_code:control.error_code},admin_control_roundtrip:{binding:roundtrip.binding,broker_rpc:roundtrip.broker_rpc,routes_readable:roundtrip.routes_readable,error_code:roundtrip.error_code},production_worker_mutated:false,production_worker_traffic_changed:false,dynamic_route_mutation:false,secrets_redacted:true},ok?200:502)
}
