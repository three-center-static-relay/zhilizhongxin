...
async function rollbackRouteReceipt(env,receipt){const rows=Array.isArray(receipt?.route_family)?receipt.route_family:[],results=[];for(const item of rows.slice().reverse()){const routeId=String(item?.route_id||"").trim(),previousVersionId=String(item?.previous_version_id||"").trim(),created=item?.created===true;if(!routeId){results.push({route:String(item?.route_name||""),ok:false,error:"ROUTE_ID_MISSING"});continue}try{if(previousVersionId){await cfRoute(env,`/routes/${encodeURIComponent(routeId)}/deployments`,{method:"POST",body:{version_id:previousVersionId}});results.push({route:String(item?.route_name||""),ok:true,restored_previous:true})}else if(created){await cfRoute(env,`/routes/${encodeURIComponent(routeId)}`,{method:"DELETE"});results.push({route:String(item?.route_name||""),ok:true,deleted_new_route:true})}else{results.push({route:String(item?.route_name||""),ok:false,error:"NO_SAFE_ROLLBACK_TARGET"})}}catch(error){results.push({route:String(item?.route_name||""),ok:false,error:safe(error?.message||error)})}}return{ok:results.length>0&&results.every(x=>x.ok),results,secrets_redacted:true}}
