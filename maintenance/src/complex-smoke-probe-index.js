...
export default{async fetch(request,env){if(!auth(request,env))return json({ok:false,error:"NOT_FOUND"},404);const mode=String(env.COMPLEX_STAGE_MODE||"brain");return json({ok:true,mode,result:{ok:true,smoke:true,candidate_reachable:true},secrets_redacted:true})}};
