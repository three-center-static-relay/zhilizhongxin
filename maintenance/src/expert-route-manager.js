...
async function runtimeTelemetry(env,fetchImpl=fetch){
  ...
  for(let page=1;page<=4;page++){
    ...
    for(const row of rows){
      ...
      const status=Number(row?.status_code||row?.status||0),success=row?.success===true||(status>=200&&status<400),text=JSON.stringify([row?.error,row?.message,row?.description,row?.internal_code,row?.internalCode]).toLowerCase(),timeout=status===504||text.includes("timeout")||text.includes("2014"),chatIncompatible=text.includes("2020")||text.includes("does not support chat completion"),server=status>=500&&status<600,key=`${provider}::${model.toLowerCase()}`,s=stats.get(key)||{n:0,ok:0,timeout:0,server:0,chatIncompatible:0};
      s.n++;if(success)s.ok++;if(timeout)s.timeout++;if(server)s.server++;if(chatIncompatible)s.chatIncompatible++;stats.set(key,s);
    }
    if(rows.length<50)break;
  }
  const quarantine=new Set();
  for(const[key,s]of stats){const rate=s.n?s.ok/s.n:1;if(s.chatIncompatible>0||s.timeout>=RUNTIME_MIN_TIMEOUTS||(s.n>=RUNTIME_MIN_SAMPLES&&s.server>=2&&rate<RUNTIME_MAX_SUCCESS_RATE))quarantine.add(key)}
...
function staticBaseScore(c){const h=c?.hints||{};return 0.40*clamp(h.quality)+0.11*clamp(h.context)+0.07*clamp(h.popularity)+0.13*clamp(h.latency)+0.11*clamp(h.throughput)+0.18*clamp(h.price)}
function staticScore(c,mode="balanced"){const base=staticBaseScore(c),price=clamp(c?.hints?.price),free=c?.free?1:0,quality=clamp(c?.hints?.quality);if(mode==="free")return 0.68*base+0.22*free+0.10*price;if(mode==="quality")return 0.80*base+0.20*quality;return base}
...
function runtimeReselectLanes(baseLanes,rows){
  const baseByCompany=new Map(baseLanes.map((lane,i)=>[lane.company,{lane,rank:i}])),byCompany=new Map();for(const c of rows){if(!byCompany.has(c.company))byCompany.set(c.company,[]);byCompany.get(c.company).push(c)}
  const companies=[...byCompany.entries()].map(([company,candidates])=>{const providerCount=new Set(candidates.map(x=>norm(x.provider))).size,baseRank=baseByCompany.get(company)?.rank??Number.MAX_SAFE_INTEGER,rawBest=Math.max(...candidates.map(x=>staticScore(x,"balanced"))),resilienceBonus=Math.min(0.06,Math.max(0,providerCount-1)*0.025);return{company,candidates,providerCount,baseRank,best:rawBest+resilienceBonus}}).sort((a,b)=>Number(b.providerCount>=2)-Number(a.providerCount>=2)||b.providerCount-a.providerCount||a.baseRank-b.baseRank||b.best-a.best).slice(0,MAX_LANES);
  if(companies.length<2)throw new Error("EXPERT_ROUTE_INSUFFICIENT_EXECUTABLE_COMPANIES_AFTER_RUNTIME_QUARANTINE");
  return companies.map((x,i)=>{const prior=baseByCompany.get(x.company)?.lane,allowed=new Set(x.candidates.map(providerKey)),keep=list=>Array.isArray(list)?list.filter(c=>allowed.has(providerKey(c))):[];let quality=prior?keep(prior.quality):[],balanced=prior?keep(prior.balanced):[],free=prior?keep(prior.free):[];if(!quality.length)quality=rankStatic(x.candidates,"quality");if(!balanced.length)balanced=rankStatic(x.candidates,"balanced");if(!free.length)free=rankStatic(x.candidates,"free",c=>c.free);return{lane:String(i+1),company:x.company,candidates:x.candidates,provider_count:x.providerCount,quality,balanced,free}})
}

export async function buildExpertRoutePlan(env={},fetchImpl=fetch){
  const base=await buildBaseExpertRoutePlan(env,fetchImpl),telemetry=await runtimeTelemetry(env,fetchImpl),quarantine=telemetry.quarantine,routeCandidates=base.routeCandidates.filter(c=>!quarantine.has(providerKey(c))),lanes=quarantine.size?runtimeReselectLanes(base.lanes,routeCandidates):base.lanes,routes=routeShards(lanes.length).map(shard=>buildRouteShard(shard,lanes)).filter(Boolean),routingFingerprint=await digest(routes.map(r=>({routeName:r.routeName,elements:r.elements}))),summary={...base.summary,schema:"expert-route-plan-v13-runtime-reselect",provider_execution_policy:"ai-gateway-provider-config-plus-live-health-plus-model-terminal-quarantine-plus-chat-compat-quarantine-plus-model-runtime-quarantine",model_runtime_quarantine:true,chat_completion_compat_quarantine:true,runtime_lane_reselection:true,runtime_reselection_applied:quarantine.size>0,runtime_quarantine_count:quarantine.size,runtime_quarantine_samples:telemetry.samples,runtime_quarantine_reason:telemetry.readable?(quarantine.size?"RUNTIME_OR_CHAT_COMPAT_FAILURE":"NO_RUNTIME_OR_CHAT_COMPAT_FAILURE"):"TELEMETRY_UNAVAILABLE",runtime_quarantine_policy:"chat-incompatible-2020>=1 OR timeouts>=2 OR samples>=3-and-server-failures>=2-and-success-rate<0.34",effective_model_timeout_ms:GATEWAY_MODEL_TIMEOUT_MS,fallback_budget_policy:"quality<=60s-balanced<=90s-free-first<=120s-before-overhead",candidate_count:routeCandidates.length,company_count:new Set(routeCandidates.map(x=>x.company)).size,lanes:lanes.map(l=>({lane:l.lane,company:l.company,candidate_count:l.candidates.length,provider_count:l.provider_count,providers:[...new Set(l.candidates.map(x=>x.provider))]})),routes:routes.map(r=>({route_name:r.routeName,lane_min:r.lane_min,lane_max:r.lane_max,lanes:r.lanes,element_count:r.element_count,model_count:r.model_count,conditional_count:r.conditional_count}))};
...
export async function refreshExpertRoutes(env={},fetchImpl=fetch,preparedPlan=null){const plan=preparedPlan||await buildExpertRoutePlan(env,fetchImpl),receipt=await refreshBaseExpertRoutes(env,fetchImpl,plan);return{...receipt,schema:"expert-route-refresh-v13-runtime-reselect",provider_execution_policy:plan.summary?.provider_execution_policy||receipt.provider_execution_policy,model_runtime_quarantine:true,chat_completion_compat_quarantine:true,runtime_lane_reselection:true,runtime_reselection_applied:plan.summary?.runtime_reselection_applied===true,runtime_quarantine_count:Number(plan.summary?.runtime_quarantine_count||0),runtime_quarantine_samples:Number(plan.summary?.runtime_quarantine_samples||0),effective_model_timeout_ms:GATEWAY_MODEL_TIMEOUT_MS,fallback_budget_policy:plan.summary?.fallback_budget_policy||null}}