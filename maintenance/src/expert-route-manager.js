...
export async function refreshExpertRoutes(env={},fetchImpl=fetch,preparedPlan=null){const plan=preparedPlan||await buildExpertRoutePlan(env,fetchImpl),receipt=await refreshBaseExpertRoutes(env,fetchImpl,plan);return{...receipt,schema:"expert-route-refresh-v12-chat-compat-runtime-health",provider_execution_policy:plan.summary?.provider_execution_policy||receipt.provider_execution_policy,model_runtime_quarantine:true,chat_completion_compat_quarantine:true,runtime_quarantine_count:Number(plan.summary?.runtime_quarantine_count||0),runtime_quarantine_samples:Number(plan.summary?.runtime_quarantine_samples||0),effective_model_timeout_ms:GATEWAY_MODEL_TIMEOUT_MS,fallback_budget_policy:plan.summary?.fallback_budget_policy||null}}
